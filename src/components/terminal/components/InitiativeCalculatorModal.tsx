...
export default function InitiativeCalculatorModal({ isOpen, onClose, baseInitiative }: InitiativeCalculatorModalProps) {
    ...
    const handleRoll = () => {
        ...
        setTimeout(() => {
            ...
            const currentModifier = INITIATIVE_MODIFIERS[selectedModifier] || INITIATIVE_MODIFIERS.none;
            const modifierValue = currentModifier.value;
            const custom = parseInt(customModifier) || 0;
            const safeBase = typeof baseInitiative === 'number' && !isNaN(baseInitiative) ? baseInitiative : 0;
            const total = roll + safeBase + modifierValue + custom;
    ...
    const currentModifier = INITIATIVE_MODIFIERS[selectedModifier] || INITIATIVE_MODIFIERS.none;
    const modifierValue = currentModifier.value;
    const safeBaseInitiative = typeof baseInitiative === 'number' && !isNaN(baseInitiative) ? baseInitiative : 0;
    const customValue = parseInt(customModifier) || 0;
    const totalModifier = safeBaseInitiative + modifierValue + customValue;

    return (
        <Modal
            isOpen={isOpen}
            onClose={handleClose}
            title="Cálculo de Iniciativa"
            className="attribute-roll-modal"
        >
                    ...
                    <>
                        <div className="roll-attribute-info compact" style={{
                            ...
                            <div style={{
                                ...
                                1d100 + {safeBaseInitiative} + <span style={{ color: (modifierValue + customValue) < 0 ? '#ef4444' : '#10b981' }}>
